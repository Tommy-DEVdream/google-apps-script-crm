var AuthService = (function() {
  function createDefaultAdminIfMissing(username, password, role) {
    ...
    if (!verifyPassword(password, existing.passwordSalt, existing.passwordHash)) {
      var reset = hashPassword(password);
      patch.passwordHash = reset.passwordHash;
      patch.passwordSalt = reset.passwordSalt;
    }

    DbService.updateById('USERS', 'userId', existing.userId, patch);
  }
